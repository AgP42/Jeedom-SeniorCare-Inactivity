...
class seniorcareinactivity extends eqLogic {
    ...
    public static function buttonAlert($_option) { // fct appelée par le listener des buttons d'alerte, n'importe quel bouton arrive ici
      log::add('seniorcareinactivity', 'debug', '################ Detection d\'un trigger d\'un bouton d\'alerte ############');

      $seniorcareinactivity = seniorcareinactivity::byId($_option['seniorcareinactivity_id']); // on cherche la personne correspondant au bouton d'alerte
      $seniorcareinactivity->execActions('action_alert_bt'); // on appelle les actions definies pour cette personne pour les boutons d'alertes
    ...
    public static function buttonAlertCancel($_option) { // fct appelée par le listener des buttons d'annulation d'alerte, n'importe quel bouton arrive ici
      log::add('seniorcareinactivity', 'debug', '################ Detection d\'un trigger d\'un bouton d\'annulation d\'alerte ############');

      $seniorcareinactivity = seniorcareinactivity::byId($_option['seniorcareinactivity_id']); // on cherche la personne correspondant au bouton d'alerte
      $seniorcareinactivity->execActions('action_cancel_alert_bt'); // on appelle les actions definies pour cette personne pour les boutons d'alertes
    ...
    public static function sensorSecurity($_option) { // fct appelée par le listener des capteurs de sécurité, n'importe quel capteur arrive ici
      log::add('seniorcareinactivity', 'debug', '################ Detection d\'un trigger de sécurité ############');

      $seniorcareinactivity = seniorcareinactivity::byId($_option['seniorcareinactivity_id']); // on cherche la personne correspondant au bouton d'alerte
      foreach ($seniorcareinactivity->getConfiguration('security') as $security) { // on boucle direct dans la conf
        if ('#' . $_option['event_id'] . '#' == $security['cmd']) { // on cherche quel est l'event qui nous a déclenché pour pouvoir chopper son nom et type (utilisé pour les tags)
          ...
          log::add('seniorcareinactivity', 'debug', 'boucle capteurs security, name : ' . $security['name'] . ' - cmd : ' . $security['cmd']  . ' - ' . $security['sensor_security_type']);
          $seniorcareinactivity->execActions('action_security', $security['name'], $security['sensor_security_type']); // on appelle les actions definies pour cette personne
    ...
    public static function sensorSecurityCancel($_option) { // fct appelée par le listener des boutons d'annulation de l'alerte de sécurité, n'importe quel capteur arrive ici
      log::add('seniorcareinactivity', 'debug', '################ Detection d\'un bouton d\'annulation d\'alerte de sécurité ############');

      $seniorcareinactivity = seniorcareinactivity::byId($_option['seniorcareinactivity_id']); // on cherche la personne correspondant au bouton d'alerte
      $seniorcareinactivity->execActions('action_cancel_security'); // on appelle les actions definies pour cette personne

    }

    public static function checkAndActionSeuilsSensorConfort($seniorcareinactivity, $_name, $_cmd, $_seuilBas, $_seuilHaut, $_type) { // appelée soit par le cron15, soit par un listener (via la fct sensorConfort - desactivée), va regarder si on est dans les seuils définis et si non appliquer les actions voulues
    ...
    // TODO on pourrait ajouter la date de collecte de la valeur pour ne pas faire des alertes sur une vieille info, ou au contraire ajouter une alerte si pas de valeur fraiche pendant un certain temps. Mais ça peut etre aussi géré par le core dans les configuration de la cmd...
      ...
      $now = time();
      $rep_warning = $seniorcareinactivity->getConfiguration('repetition_warning');
      $tempsDepuisActionWarningConfort = $now - $seniorcareinactivity->getCache('actionWarningConfortStartTimestamp' . $_cmd); // on garde 1 cache par cmd
      $warningConfortLauched = $seniorcareinactivity->getCache('WarningConfortLauched' . $_cmd);

      $valeur = jeedom::evaluateExpression($_cmd);

      log::add('seniorcareinactivity', 'debug', 'Fct checkAndActionSeuilsSensorConfort, name : ' . $_name . ' - ' . $_type . ' - ' . $_cmd . ' - ' . $valeur . ' - ' . $_seuilBas . ' - ' . $_seuilHaut . ' - Rep warning : ' . $seniorcareinactivity->getConfiguration('repetition_warning'));

      log::add('seniorcareinactivity', 'debug', 'Fct checkAndActionSeuilsSensorConfort, WarningConfortLauched : ' . $warningConfortLauched . ' - last action lancé il y a (min) : ' . $tempsDepuisActionWarningConfort / 60);

      if(!is_numeric($valeur)){

        log::add('seniorcareinactivity', 'debug', 'Capteurs confort :' . $_name . ' la valeur est pas numerique, on fait rien !');
        ...
        log::add('seniorcareinactivity', 'debug', 'Capteurs confort :' . $_name . ' dans les seuils, on fait rien');
        ...
        log::add('seniorcareinactivity', 'debug', 'Capteurs confort :' . $_name . ' retour à la normal !');
        $seniorcareinactivity->setCache('WarningConfortLauched' . $_cmd, false); // on remet dans le cache qu'on a pas lancé les actions
        $seniorcareinactivity->execActions('action_cancel_warning_confort', $_name, $_type, $valeur, $_seuilBas, $_seuilHaut); // appel de la boucle d'execution des actions avec les infos pour les tag des messages
        return 1;
        ...
        log::add('seniorcareinactivity', 'debug', 'Capteurs confort :' . $_name . ' est hors seuils, tempsDepuisActionWarningConfort : ' . $tempsDepuisActionWarningConfort . ' warningConfortLauched : ' . $warningConfortLauched . ' rep_warning : ' . $rep_warning);

        $seniorcareinactivity->setCache('WarningConfortLauched' . $_cmd, true); // on garde en cache qu'on a lancé nos actions au moins 1 fois pour cette commande
        $seniorcareinactivity->setCache('actionWarningConfortStartTimestamp' . $_cmd, $now); // on memorise l'heure du lancement du warning

        $seniorcareinactivity->execActions('action_warning_confort', $_name, $_type, $valeur, $_seuilBas, $_seuilHaut); // on execute toutes les actions
        ...
        log::add('seniorcareinactivity', 'debug', 'Capteurs confort :' . $_name . ' est hors seuils, mais déjà lancé les actions de warning');
...
/*    public static function sensorConfort($_option) { // fct appelée par le listener des capteurs conforts (on sait pas lequel, ça serait trop simple, mais on connait l'event_id et la valeur).
      ...
      log::add('seniorcareinactivity', 'debug', '################ Detection d\'un changement d\'un capteur confort ############');

    //  log::add('seniorcareinactivity', 'debug', 'Fct sensorConfort appelé par le listener : $_option[seniorcareinactivity_id] : ' . $_option['seniorcareinactivity_id'] . ' - value : ' . $_option['value'] . ' - event_id : ' . $_option['event_id']);

      $seniorcareinactivity = seniorcareinactivity::byId($_option['seniorcareinactivity_id']);
      if (is_object($seniorcareinactivity) && $seniorcareinactivity->getIsEnable() == 1 ) {
        foreach ($seniorcareinactivity->getConfiguration('confort') as $confort) { // on boucle direct dans la conf
          if ('#' . $_option['event_id'] . '#' == $confort['cmd']) { // on cherche quel est l'event qui nous a déclenché

          //  log::add('seniorcareinactivity', 'debug', 'Fct sensorConfort appelé par le listener, name : ' . $confort['name'] . ' - cmd : ' . $confort['cmd']  . ' - ' . $confort['sensor_confort_type'] . ' - ' . $confort['seuilBas'] . ' - ' . $confort['seuilHaut']);

            if($confort['seuilBas'] != '' || $confort['seuilHaut'] != '') { // si les seuils sont definis (on set le listener de toutes facons maintenant)
              $seniorcareinactivity->checkAndActionSeuilsSensorConfort($seniorcareinactivity, $confort['name'], $_option['value'], $confort['seuilBas'], $confort['seuilHaut'], $confort['sensor_confort_type']);
            }
    ...
    // Sert ici pour les capteurs conforts
      public static function cron15() {
        ...
        log::add('seniorcareinactivity', 'debug', '#################### CRON 15 ###################');
        ...
        foreach (self::byType('seniorcareinactivity',true) as $seniorcareinactivity) {
          ...
          if (is_object($seniorcareinactivity) && $seniorcareinactivity->getIsEnable() == 1) { // si notre eq existe et est actif
            ...
            foreach ($seniorcareinactivity->getConfiguration('confort') as $confort) { // on boucle direct dans la conf
              ...
              log::add('seniorcareinactivity', 'debug', 'Cron15 boucle capteurs confort, name : ' . $confort['name'] . ' - cmd : ' . $confort['cmd']  . ' - ' . $confort['sensor_confort_type'] . ' - ' . $confort['seuilBas'] . ' - ' . $confort['seuilHaut']);
              ...
              if($confort['seuilBas'] != '' || $confort['seuilHaut'] != '') { // évalue si on a au moins 1 seuil defini (de toute facon on peut pas n'en remplir qu'1 des deux)
                ...
                $etatSensor *= $seniorcareinactivity->checkAndActionSeuilsSensorConfort($seniorcareinactivity, $confort['name'], $confort['cmd'], $confort['seuilBas'], $confort['seuilHaut'], $confort['sensor_confort_type']);
                log::add('seniorcareinactivity', 'debug', 'Cron15 boucle capteurs confort, etatSensor : ' . $etatSensor);
            ...
            if($etatSensor){ // ils ont tous repondu 1, on va lancer les actions
              $seniorcareinactivity->execActions('action_cancel_all_warning_confort'); // appel de la boucle d'execution des actions avec les infos pour les tag des messages
            }
    ...
    //    log::add('seniorcareinactivity', 'debug', 'cleanAllListener id lue : ' . $seniorcareinactivity_id_listener . ' et nous on est l id : ' . $this->getId());
...
/*      $listener = listener::byClassAndFunction('seniorcareinactivity', 'sensorLifeSign', array('seniorcareinactivity_id' => intval($this->getId())));
      if (is_object($listener)) {
        $listener->remove();
      }
      $listener = listener::byClassAndFunction('seniorcareinactivity', 'buttonAlert', array('seniorcareinactivity_id' => intval($this->getId())));
      if (is_object($listener)) {
        $listener->remove();
      }
      $listener = listener::byClassAndFunction('seniorcareinactivity', 'sensorConfort', array('seniorcareinactivity_id' => intval($this->getId())));
      if (is_object($listener)) {
        $listener->remove();
      }
      $listener = listener::byClassAndFunction('seniorcareinactivity', 'sensorSecurity', array('seniorcareinactivity_id' => intval($this->getId())));
      if (is_object($listener)) {
        $listener->remove();
      } //*/
...
class seniorcareinactivityCmd extends cmd {
   ...
   /*   if ($this->getLogicalId() == 'sensor_confort') {
        log::add('seniorcareinactivity', 'debug', 'Fct execute - Capteurs confort, valeur renvoyée : ' . round(jeedom::evaluateExpression($this->getValue()), 1));
      ...
      if ($this->getLogicalId() == 'sensor_alert_bt' || $this->getLogicalId() == 'sensor_life_sign' || $this->getLogicalId() == 'sensor_security') {
        log::add('seniorcareinactivity', 'debug', 'Fct execute - sensor_alert_bt or sensor_life_sign or sensor_security, valeur renvoyée : ' . jeedom::evaluateExpression($this->getValue()));
        return jeedom::evaluateExpression($this->getValue());
      } //*/
