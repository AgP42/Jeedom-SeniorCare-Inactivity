...
Il s'agit ici de déclarer les capteurs concernant l’activité de la personne (ouverture, interrupteur, mouvement, …) et un délai associé pour chacun. Si aucun capteurs d'activité n'a été activé à l’échéance du délai, le plugin déclenchera l’étape suivante « Actions d'alertes ».
...
* Pour les capteurs de "détections d'inactivité", c'est le changement de valeur du capteur qui est détecté et relancera la temporisation, la valeur en elle-même n'est pas prise en compte !
   * Pour un capteur de mouvement, cela signifie que la détection d'un changement d'état quel qu'il soit, passage du capteur de actif à inactif ou de inactif à actif, déclenchera la temporisation
